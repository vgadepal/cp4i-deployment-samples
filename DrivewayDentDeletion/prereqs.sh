...
export ADMIN_USER_CS=admin
export SYSTEM_ADMIN_USER=system:admin
mkdir -p ${PWD}/tmp
echo "Fetching kubeconfig of cluster and creating secret"
oc adm policy add-role-to-user admin $ADMIN_USER_CS -n $NAMESPACE
oc adm policy add-role-to-user admin $SYSTEM_ADMIN_USER -n $NAMESPACE
...
if ! ./security/secret.sh; then
  echo "Failed to create secrets and key store"
  exit 1
fi
...
# export QM_KEY=$(cat security/server.key | base64 | tr -d '\n')
# export QM_CERT=$(cat security/server.crt | base64 | tr -d '\n')
# export APP_CERT=$(cat security/application.crt | base64 | tr -d '\n')

# cat << EOF | oc apply --namespace mq -f -
# kind: Secret
# apiVersion: v1
# metadata:
#   name: mqcert
...
#dynamically generating tls key n crt pair
openssl req -newkey rsa:2048 -nodes -keyout tls.key -x509 -days 1000 -out tls.crt -subj "/C=US/ST=New York/L=Armonk/O=IBM Cloud Private/CN=admin"
oc create secret generic -n mq mqcert --from-file=tls.crt=tls.crt --from-file=tls.key=tls.key
...
#   --from-literal=truststoreCert-mykey= \
#   --from-literal=odbcini= \
#   --from-file=serverconf=$PWD/security/server.conf.yaml \
#   --from-literal=setdbparms= \
#   --from-literal=extensions=$PWD/security/extensions.zip \
#   --from-file=policy=/tmp/policy.xml \
#   --from-file=policyDescriptor=/tmp/policyDescriptor.xml \
#   --dry-run -o yaml | oc apply -f -
